...
using ECommerce.Modules.Sales.Features.CreateOrder;
...
builder.Services.AddDbContext<CatalogDbContext>(options =>
{
    options.UseSqlite(connectionString,
        o => o.MigrationsAssembly("ECommerce.Modules.Catalog"));
} );

builder.Services.AddDbContext<SalesDbContext>(options =>
{
    options.UseSqlite(salesConnectionString,
        o => o.MigrationsAssembly("ECommerce.Modules.Sales"));
});

builder.Services.AddDbContext<CartDbContext>(options =>
{
    options.UseSqlite(cartConnectionString,
        o => o.MigrationsAssembly("ECommerce.Modules.Cart"));
});
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.WithTitle("My Modern API")
               .WithTheme(ScalarTheme.DeepSpace); // Optional: customize style
    });
}
...
app.MapPost("/sales/orders", async (CreateOrderCommand command, IMessageBus bus) =>
{
    return await bus.InvokeAsync<CreateOrderResponse>(command);
})
...
app.MapGet("/cart/{customerId:int}", async (int customerId, IMessageBus bus) =>
{
...
app.MapPost("/cart/items", async (AddToCartCommand command, IMessageBus bus) =>
{
...
app.MapPut("/cart/items", async (UpdateCartItemCommand command, IMessageBus bus) =>
{
    var cart = await bus.InvokeAsync<CartDto?>(command);
    return cart is null ? Results.NotFound() : Results.Ok(cart);
})
...
app.MapDelete("/cart/{customerId:int}/items/{productId:int}", async (
    int customerId,
    int productId,
    IMessageBus bus) =>
{
    var cart = await bus.InvokeAsync<CartDto?>(new RemoveFromCartCommand(customerId, productId));
    return cart is null ? Results.NotFound() : Results.Ok(cart);
})
.WithName("RemoveFromCart")
.WithTags("Cart");

app.MapGet("/catalog/homepage", async (IMessageBus bus) =>
{
...
app.MapPost("/catalog/products", async (CreateProductCommand command, IMessageBus bus) =>
{
...
app.MapGet("/catalog/products/{id:int}", async (int id, IMessageBus bus) =>
{
...
app.MapPost("/catalog/categories", async (CreateCategoryCommand command, IMessageBus bus) =>
{
...
app.MapGet("/catalog/categories/{id:int}", async (int id, IMessageBus bus) =>
{
...
app.MapGet("/catalog/Search", async (string? term, IMessageBus bus) =>
{
